...
class DraggableHTMLElement extends HTMLElement {
  ...
  handleDrop(event) {
    ...
    this.classList.remove("drag-top", "drag-bottom");
    if (
      DraggableHTMLElement.currentDrag &&
      DraggableHTMLElement.currentDrag !== this
    ) {
      const currentParent = this.parentNode;
      if (
        DraggableHTMLElement.originalParent.tagName === currentParent.tagName
      ) {
        const targetRect = this.getBoundingClientRect();
        const halfwayPoint = targetRect.top + targetRect.height / 2;
...
class Option extends HTMLElement {
  ...
  fromJson() {
    console.log("fromJson not implemented", this);
  }

  toJson() {
    console.log("toJson not implemented");
  }
}
...
class DocumentationOption extends Option {
  ...
  togglePreview() {
    ...
    if (this.isPreview) {
      this.previewDiv.innerHTML = DOMPurify.sanitize(
        marked.parse(this.textarea.getValue()),
      );
    }
    this.textarea.classList.toggle("hidden", this.isPreview);
    this.previewDiv.classList.toggle("hidden", !this.isPreview);
    this.toggleButton.setSrc(
      this.isPreview ? "assets/edit.svg" : "assets/article.svg",
    );
  }
...
class VariableEditor extends HTMLElement {
  constructor(
    languages = ["ENG", "SP", "GER", "FR", "IT", "KOR", "JP5", "JP6"],
  ) {
    super();
  ...
  getValue() {
    ...
    return (
      this.overrulingValues.get(language).getValue() || this.default.getValue()
    );
  }
  ...
  fromJson(json) {
    ...
    for (const language in json.overrulingValues) {
      this.overrulingValues
        .get(language)
        .setValue(json.overrulingValues[language]);
    }
  }
...
class Variable extends DraggableHTMLElement {
  constructor(varEvaluator) {
    ...
    document.addEventListener(
      "languageUpdate",
      this.updateHeaderValue.bind(this),
    );
  }
...
class VariablesOption extends Option {
  ...
  getParsableVariables() {
    ...
    for (const variable of json) {
      ...
      for (const language in variable.overrulingValues) {
        variableJson[language.toLowerCase()] =
          variable.overrulingValues[language];
      }
  ...
  mathEvaluation(expression, returnExpression) {
    try {
      ...
      console.error(
        "Error evaluating as mathExpression:",
        expression,
        ":",
        evaluationError,
      );
...
class MemoryEditorBlock extends DraggableHTMLElement {
  ...
  getActualSize(sizeString) {
    ...
    if (!sizeString || size > this.maxByteCount) return this.maxByteCount;
  ...
  toJson(parsed = false) {
    ...
    for (let i = 0; i < size; i++) {
      if (memoryInputs[i].isActive()) {
        if (parsed) {
          values.push(
            this.varEvaluator.evaluateString(memoryInputs[i].getValue()),
          );
        } else {
...
class CommandInput extends InputElement {
  ...
  toJson(parsed = false) {
    if (parsed) {
      const commandData = CommandDataList.instance.parse(
        this.evaluateString(this.getValue(), true),
      );
...
class ParamList extends HTMLElement {
  ...
  updateCommandInfo(command) {
    ...
    if (!commandInfo || commandInfo == this.currentCommandInfo) return;
  ...
  fromCommand(command) {
    if (!this.updateCommandInfo(command)) return;
  ...
  toJson(parsed = false) {
    ...
    for (const param of this.children) {
      ...
      if (parsed) {
        ...
        paramJson["value"] = this.varEvaluator.evaluateString(
          param.input.value,
        );
      } else {
...
class CommandBlock extends DraggableHTMLElement {
  constructor(varEvaluator) {
    ...
    this.command.onEvaluation(
      this.parameters.fromCommand.bind(this.parameters),
    );
...
class CodeCreateBlock extends HTMLElement {
  constructor(
    options = {
      "Add Command": "command",
      "Add Memory Editor": "memory_editor",
...
class ScriptOptionList extends HTMLElement {
  constructor() {
    ...
    optionContainer.append(this.documentation, this.variables, this.code);
...
class Script extends HTMLElement {
  constructor() {
    ...
    this.colorSwatch.onColorChange(
      this.optionList.code.setColor.bind(this.optionList.code),
    );
  ...
  dispatchSelectEvent() {
    if (this.enabled) {
      document.dispatchEvent(
        new CustomEvent("scriptSelected", { detail: this }),
      );
    }
  }
...
class ScriptGroupListWrapper extends HTMLElement {
  constructor() {
    ...
    this.onCreateScript(
      this.scriptGroups.addScriptGroup.bind(this.scriptGroups),
    );
    this.append(this.scriptGroups, this.create);
  }
...
class ScriptGroupManager extends HTMLElement {
  constructor() {
    ...
    this.append(this.scriptGroupListWrapper, this.scriptGroupArea);
